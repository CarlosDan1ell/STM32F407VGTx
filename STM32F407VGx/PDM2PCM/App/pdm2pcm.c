...
uint8_t MX_PDM2PCM_Process(uint16_t *PDMBuf, uint16_t *PCMBuf)
{
  ...
  */
  /* this example return the default status AUDIO_ERROR */
	for (int i = 0; i < PDM1_filter_handler.out_ptr_channels; i++)
	  {
	    PDM_Filter(&PDMBuf[i * PDM1_filter_config.decimation_factor * PDM1_filter_config.output_samples_number / 8],
	               &PCMBuf[i * PDM1_filter_config.output_samples_number],
	               &PDM1_filter_handler);
	  }
