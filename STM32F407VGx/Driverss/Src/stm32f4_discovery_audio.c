...
extern PDM_Filter_Config_t   PDM1_filter_config[2];
...
static void PDMDecoder_Init(uint32_t AudioFreq, uint32_t ChnlNbrIn, uint32_t ChnlNbrOut)
{
  ...
  for(index = 0; index < ChnlNbrIn; index++)
  {
    ...
    PDM1_filter_config[index].output_samples_number = AudioFreq/1000;
    PDM1_filter_config[index].mic_gain = 24;
    PDM1_filter_config[index].decimation_factor = PDM_FILTER_DEC_FACTOR_64;
    PDM_Filter_setConfig((PDM_Filter_Handler_t *)&PDM_FilterHandler[index], &PDM1_filter_config[index]);
  }
}
